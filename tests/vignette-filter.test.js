...
assert.deepStrictEqual(JSON.parse(JSON.stringify(normalized)), { size: 50, strength: 2, color: "ff79b3db", fade: 0 });
assert.deepStrictEqual(JSON.parse(JSON.stringify(Vignette.normalizeVignetteParams({ size: -4, strength: 0, color: "bad", fade: 200 }))), { size: 0, strength: 1, color: "ff79b3db", fade: 100 });
...
const half = Vignette.applyVignetteToBuffer(source, width, height, { size: 50, strength: 2, color: "ff79b3db", fade: 50 });
const faded = Vignette.applyVignetteToBuffer(source, width, height, { size: 50, strength: 2, color: "ff79b3db", fade: 100 });
...
assert.deepStrictEqual(Array.from(faded), Array.from(source), "fade 100 is an exact bypass");
...
for (let i = 0; i < source.length; i += 4) {
  assert.ok(Math.abs(half[i] - Math.round((full[i] + source[i]) / 2)) <= 1, "fade 50 is the red midpoint");
  assert.ok(Math.abs(half[i + 1] - Math.round((full[i + 1] + source[i + 1]) / 2)) <= 1, "fade 50 is the green midpoint");
  assert.ok(Math.abs(half[i + 2] - Math.round((full[i + 2] + source[i + 2]) / 2)) <= 1, "fade 50 is the blue midpoint");
}
...
assert.match(html, /vignette=1,\$\{normalized\.size\},\$\{normalized\.strength\},\$\{normalized\.color\},\$\{normalized\.fade\}/, "token serializes size, strength, color, and fade");
